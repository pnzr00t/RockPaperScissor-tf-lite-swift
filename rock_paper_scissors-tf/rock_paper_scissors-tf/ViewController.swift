...
//  rock_paper_scissors-tf
//
//
//  Created by 	Oleg2 on 11.02.2020.
...
class ViewController: UIViewController {
  ...
  private var modelDataHandler: ModelDataHandler? =
    ModelDataHandler(modelFileInfo: MobileNet())
...
extension ViewController: UICollectionViewDelegate, UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    debugPrint(firstInference.confidence * 100)
    debugPrint(Int(round(firstInference.confidence * 100)))
    
    inferences[indexPath.item] = firstInference.label + " - \(String(format: "%.01f", firstInference.confidence * 100))" + "%"
    collectionView.reloadData() // Can make "Update collection cell instead reloadData"
  }
